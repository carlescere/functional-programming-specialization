...
trait Solver extends GameDef {
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    val neighbors = (for (move <- initial) yield neighborsWithHistory(move._1, move._2)).flatten
    val newMoves = newNeighborsOnly(neighbors, explored)
    val newBlocks = for (move <- newMoves) yield move._1

    if (newMoves.isEmpty) initial
    else from(initial #::: newMoves, explored ++ newBlocks)
  }
