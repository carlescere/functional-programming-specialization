...
object Anagrams {
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    val allCounts = occurrences.flatMap {
      case (char, count) => for {
        c <- 1 to count
      } yield (char, c)
    }
    (0 to allCounts.length)
      .map(allCounts.combinations)
      .reduce(_ ++ _)
      .toList
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    val values = (x ::: y).groupBy(_._1).mapValues(_ match {
      case List(j) => j._2
      case j :: k :: Nil => Math.abs(j._2 - k._2)
    })
    values.toList.filter(_._2 > 0).sorted
  }
    ...
    def occurrencesAnagrams(occurrences: Occurrences): List[Sentence] = {
      occurrences match {
        ...
        case _ => for {
          ...
          partial <- occurrencesAnagrams(subtract(occurrences, combination))
        } yield word :: partial
      }
    }
